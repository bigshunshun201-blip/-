(function (root, factory) {
  ...
  function renderPlan(view) {
    return `
      ...
      ${view.plans.length ? `<div class="quick-plan-toolbar"><div><strong>这批都不合适？</strong><span>换一批会自动保存当前三案，不会覆盖历史记录。</span></div><button type="button" data-quick-regenerate-plans ${view.hasScript ? "disabled" : ""}>${icon("refresh-cw")}换 3 套新方向</button></div>` : ""}
      ${view.planBatches?.length > 1 ? `<details class="quick-plan-history"><summary>查看历史策划批次（${view.planBatches.length}）</summary><div>${view.planBatches.map((batch) => `<button type="button" data-quick-plan-batch-restore="${escapeHtml(batch.id)}" ${batch.active ? "disabled" : ""}><span>${escapeHtml(batch.label)}</span><strong>${batch.active ? "当前批次" : "恢复这批"}</strong></button>`).join("")}</div></details>` : ""}
  ...
  function renderDrawer(type, view) {
    ...
    if (type === "topics") return { title: "从选题库找灵感", eyebrow: "选题可以整批换新，也可以只替换一条", body: `<div class="quick-drawer-toolbar"><div><strong>当前 ${view.topics.length} 个候选</strong><span>每次生成都会避开当前选题。</span></div><button type="button" data-quick-topics-regenerate>${icon("refresh-cw")}AI 换一批</button></div><div class="quick-topic-drawer">${view.topics.map((item, index) => `<article><span>${escapeHtml(item.priority || "A")}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.sellingPoint)}</p><small>${escapeHtml(item.emotion)} · ${escapeHtml(item.duration)} 秒</small><div class="quick-topic-actions"><button type="button" data-quick-topic-select="${index}">用这个方向开题</button><button type="button" data-quick-topic-replace="${index}">${icon("replace")}替换这条</button></div></article>`).join("") || `<p class="quick-empty-copy">当前没有选题。点击“AI 换一批”生成候选，也可以关闭抽屉后直接输入想法。</p>`}</div>` };
