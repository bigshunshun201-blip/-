(function () {
  ...
  function generateTopics(competitorRows = seedCompetitors) {
    ...
    const topics = [
      ["魔法学院开了邪修速成班", "把游戏升级拍成野路子训练法，天然适合搞笑语录和反差剪辑", "好笑、离谱、上头", "所谓邪修其实是被封印的古代训练法", 60, true],
      ["宠物考试全员外耗老师", "课堂整活+宠物嘴替，适合弹幕吐槽型短剧", "解压、抽象、反差", "老师才是被暗影操控的最终考题", 45, true],
      ...
      ["旧徽章每晚都会发疯文学", "道具悬疑套热梗文案，封面可打“它开始乱回我了”", "不安、好笑、使命感", "徽章不是坏了，而是在替失踪宠物求救", 60, true],
      ["全王国都说这只宠物不嘻嘻了", "萌宠反差+情绪爆点，适合搞笑开头催泪结尾", "好笑、心疼、重逢", "宠物冷脸是因为它每天都在重置记忆", 75, true],
    ...
    return topics.map((item, index) => ({
      title: item[0],
      sellingPoint: item[1],
      audience: audiences[(index + audienceOffset) % audiences.length],
      emotion: item[2],
      reversal: item[3],
      memeLine: memeLines[(index + memeOffset) % memeLines.length],
      duration: item[4],
      series: item[5],
